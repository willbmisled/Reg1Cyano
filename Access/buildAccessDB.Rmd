...
#edit depth
  table(tblSample$sampleDepthM,useNA='ifany')
  tblSample$sampleDepthM[tblSample$sampleDepthM=='surface']<-0
  tblSample$sampleDepthM<-as.numeric(tblSample$sampleDepthM)
...
#read the new data
  ...
  SELECT tempAll.organization, tempAll.waterbodyName, tempAll.otherWaterbodyID, tempAll.state, tempAll.stationLocation, tempAll.sampleDate, tempAll.fieldCrew, tempAll.weather, tempAll.stationID, tempAll.stationType, tempAll.sampleDepthM, tempAll.latitudeSta, tempAll.longitudeSta, tempAll.sampleTime, tempAll.filtered, tempAll.frozen, tempAll.parameter, tempAll.reading, tempAll.analysisDate, tempAll.analysisTime, tempAll.fluorometerType
  FROM tempAll;
...
#############compare CRWA 
  ...
  'Organization','SiteID','WaterbodyName','State','SiteLocation','SampleDate','TypeOfSample','Depth','Latitude','Longitude','SampleTime','Filtered.','Frozen.','Parameter','Value','AnalysisDate','AnalysisTime','Fluorometer'
  
  
  #select CRWA and order by fields
  



  new<-a[new$organization==CRWA,b]
  

  
#pick fields to compare
  b<-c('organization','otherWaterbodyID','waterbodyName','state','stationLocation','sampleDate','stationID','sampleDepthM','latitudeSta','longitudeSta','sampleTime','filtered','frozen','parameter','reading','analysisDate','analysisTime','fluorometerType')
...
#order by fields
  ...
  new<-a[order(a[,b]),b]
  new<-subset(new,new$organization=='CRWA')

#read the original data
  CRWA<-read.csv("../Data2014/CRWA_RAW.csv",stringsAsFactors=FALSE) #read the data

#pick fields to compare
  a<-c('Organization','SiteID','WaterbodyName','State','SiteLocation','SampleDate','TypeOfSample','Depth','Latitude','Longitude','SampleTime','Filtered.','Frozen.','Parameter','Value','AnalysisDate','AnalysisTime','Fluorometer')

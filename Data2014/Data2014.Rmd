...
To Do List
-------------------------
* Need data from UNH, RI, and ME
* Need answers to questions for CT
...
#save the data
  write.table(VTDEC, file='VTDEC.csv',row.names=F,sep=',') #write to csv
```
...
#save the data
  write.table(CRWA, file='CRWA.csv',row.names=F,sep=',') #write to csv
```
...
write.table(NHDES, file='NHDES.csv',row.names=F,sep=',') #write to csv
```
...
#Save CTDEEP
  write.table(CTDEEP, file='CTDEEP.csv',row.names=F,sep=',') #write to csv
```

### University of New Hampshire

* Data in prep by Jim Haney and Amanda Murby
...
Collated Data
-------------------------
Once the individual data sets are standardized then they are combined into a single dataset that is saved as a text file in CSV format.

```{r CTDEEP444, include=FALSE, echo=FALSE, cache=FALSE} 
  Data2014<-rbind(CRWA,CTDEEP,NHDES,VTDEC) #combine data
...
Field  | Description
------------- | ------------- 
**SiteID:**|If the location has a site ID from your organization enter it here
**WaterbodyName:**|Enter the name of the lake: be consistent with spelling and capitalization
**State:**|Choose the two letter state code from the drop down list
**SiteLocation:**|Some organizations have site names within a lake.  Enter these here.
**SampleLocation:**|Where (in the lake) was the sample collected followed by the replicate number? WithinLake=WL1, WL2, or WL3; ShoreSide=SS1, SS2, SS3; Can also add Calibration or Blank for validation readings or Other.
**Latitude:**|Latitude in Decimal Degrees with 4 decimal places or degrees minutes seconds
**Longitude:**|Longitude in Decimal Degrees with 4 decimal places or degrees minutes seconds

* Are SiteIDs unique among states?
{r xy1, include=TRUE, echo=FALSE, cache=FALSE} 
a<-data.frame(Data2014[,c('SiteID','WaterbodyName','State','SiteLocation','SampleLocation','Latitude','Longitude')])
nrow(a)
b<-unique(a)
nrow(b)
b<-b[order(b$State,b$SiteID),]

  a<-unique(data.frame(SiteID=Data2014$SiteID,ST=Data2014$State))
  b<-as.data.frame(table(a$SiteID))  
  max(b[,2])  #all unique
...
SiteID,WaterbodyName,State,SiteLocation,SampleLocation,Latitude,Longitude





unique(Data2014$SiteID)
unique(Data2014$WaterbodyName)

head(Data2014)
table(is.na(Data2014$Latitude))

a<-with(Data2014,table(SiteID,State))
b<-a[,4]
d<-data.frame(b,s=apply(a,1,sum))
d[d[,1]>0,]
...
as.data.frame(table(Data2014$Longitude,useNA='ifany'))

table(is.na(Data2014$Latitude))
table(is.na(Data2014$Longitude))

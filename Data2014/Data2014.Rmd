...
NH<-read.csv("NHDES_RAW.csv") #read the data
  NH_T<-NH[,1:30] #NH template-delete Fluoremeter observations

#loop to reformat the data into a long form
  NHDES<-c()  #empty data frame
    for(i in c(35:ncol(NH))){
      a<-unlist(strsplit(names(NH)[i],'_')) #get the column name
      b<-NH_T #open the blank template then fill in the field below
        b$Parameter<-ifelse(a[1]=='Phycocyanin','Phycocyanin','Chlorophyll')
        b$Units<-ifelse(a[2]=='RFU','RFU',paste('\u03BC','g/l',sep=''))
        b$Fluorometer<-ifelse(a[3]=='Beagle','Beagle','Turner')
        b$Frozen<-ifelse(a[4]=='Frozen','TRUE','FALSE')
        b$Filtered<-ifelse(a[5]=='Filtered','TRUE','FALSE')
        b$Value<-NH[,i]
      NHDES<-rbind(NHDES,b) #add data to data.frame
      NHDES[NHDES$SiteID=="TOWCAN-GEN","SiteID"]<-'TOWCND-GEN'
    }
...
CT<-read.csv("CTDEEP.csv") #read the data

#locations
CTDEEP_Locations<-unique(CT[,c('SiteID','WaterbodyName','SiteLocation')])
CTDEEP_Locations$Latitude<-NA
CTDEEP_Locations$Longitude<-NA
write.table(CTDEEP_Locations, file='CTDEEP_Locations.csv',row.names=F,sep=',') #write to csv
```
...
VT<-read.csv("VTDEC.csv") #read the data
```

...
libs<-c("rgdal","sp","knitr","maptools","plotKML") #list of packages to load
...
To Do List
-------------------------
* missing location data for ME, NH, and UNH
...
#which locations are missing
  table(is.na(a$Latitude)) #188 missing
  table(a[is.na(a$Latitude),'Organization'])  #NHDES=128 UNH_CFB=60
  Missing<-a[is.na(a$Latitude),]
  table(Missing$SiteID,useNA='ifany') #This shows the NHDES that are missing UNH are the NA's
#missing UNH sites
  unh<-Missing[Missing$Organization=='UNH_CFB',]
  table(unh$WaterbodyName,useNA='ifany')
  table(unh$SiteLocation,useNA='ifany')
  table(unh$SampleLocation,useNA='ifany') #all NA
  unhMissing<-unique(unh[,c('WaterbodyName','SiteLocation','Latitude','Longitude')])
    #save the data
  write.table(unhMissing, file='UNH_MissingLocations.csv',row.names=F,sep=',') #write to csv
```
